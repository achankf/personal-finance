...
use crate::{
    one_shot::get_trade_data::TradeType, AcbEntry, AcbEntryType, GlBreakDown, MyBigDecimal,
};
...
impl TradeData {
    ...
    pub fn get_single_breakdown(
        ...
        let disposition_capital_gl = self
            ...
            .fold(MyBigDecimal::zero(), |acc, cur| acc + cur.capital_gl)
            .round9();
        ...
        self.distribution_ref_map
            ...
            .fold(init_value, |acc, entry| {
                let unit = self
                    ...
                    .unwrap_or_default();
                let balance = |x: &MyBigDecimal| x.clone() * unit.clone();

                let unit_capital_gain = entry.capital_gain.clone()
                    + entry.capital_gain_pre_20240624.clone()
                    + entry.capital_gain_post_20240624.clone();
                ...
                acc + GlBreakDown {
                    ...
                    distribution_capital_gain: balance(&unit_capital_gain),
                    ...
                    foreign_business_income_tax_paid: balance(
                        &entry.foreign_business_income_tax_paid,
                    ),
                    foreign_non_business_income_tax_paid: balance(
                        &entry.foreign_non_business_income_tax_paid,
                    ),
    ...
    pub fn get_acb(&self, person_id: i64, security_id: i64) -> Vec<AcbEntry> {
        ...
        let mut acc_acb = MyBigDecimal::zero();
        ...
        {
            let amount = (unit.clone() * price.clone()).round9();

            let (acb_change, capital_gl): (MyBigDecimal, MyBigDecimal) = match r#type {
            ...
            let acb_change = acb_change.round9();
