...
SELECT
    ...
    (
        capital_gain + eligible_dividend + foreign_non_business_income + other_income + return_of_capital + non_eligible_dividend + foreign_business_income + non_reportable_distribution + capital_gains_eligible_for_deduction
    ) AS "taxable_gain!:f64",
    (
...
FROM
    ...
    FULL OUTER JOIN (
        SELECT
            ...
            1 AS unused_flag
        FROM
            UnusedDistributionData
    ) USING (security_id, payment_date)
WHERE
    -- ignore distributions that haven't been paid
