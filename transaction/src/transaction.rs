...
impl<'c> MyTransaction<'c> {
    pub async fn try_new(target: &'c mut db::Db) -> db::SqlResult<MyTransaction<'c>> {
        Ok(MyTransaction(target.begin().await?))
    }
...
impl<'c> std::ops::Deref for MyTransaction<'c> {
...
impl std::ops::DerefMut for MyTransaction<'_> {
