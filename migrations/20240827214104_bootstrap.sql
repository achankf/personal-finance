...
INSERT INTO AccountKind(account_kind)
VALUES ('ASSET'),
    ('EQUITY'),
    ('EXPENSE'),
    ('LIABILITIES'),
...
INSERT INTO AccountSubtypePrepopulated (account_subtype, account_kind)
VALUES ('CASH', 'ASSET'),
    -- changes made with cash transactions
    ('CHANGE', 'REVENUE'),
    ('FEES', 'EXPENSE'),
    ('INTEREST', 'REVENUE'),
    ...
    ('CASHBACK', 'REVENUE'),
    ('DEPOSIT-REVERSAL', 'EXPENSE'),
    -- rejected entries (for example to avoid counting salary when the cheque-in deposit is being reverse)
    -- these are dummy entries that are needed to make the book balanced but are separated from the real accounts
    ('REJECTED-EXPENSE', 'EXPENSE'),
    ('REJECTED-REVENUE', 'REVENUE'),
    ...
    ('SALARY', 'REVENUE'),
    ('WORK-PENALTY', 'EXPENSE'),
    ('INCOME-TAX', 'EXPENSE'),
    ('REIMBURSEMENT', 'REVENUE'),
    ('VACATION-PAY', 'REVENUE'),
    ('TERMINATION-PAY', 'REVENUE');

INSERT INTO AccountSubtype (account_subtype, account_kind_id)
SELECT account_subtype,
    account_kind_id
FROM AccountSubtypePrepopulated
...
CREATE TABLE FinancialEntry (
    ...
    -- when empty string forex_rate means the main currency (CAD) at 1-to-1 exchange rate
    forex_rate TEXT,
...
INSERT INTO CashAccountEntryType (account_subtype_id)
SELECT account_subtype_id
FROM AccountSubtype
WHERE account_subtype IN (
        'CASH',
        'FEES',
        'INTEREST',
        'OPEN-BALANCE',
...
INSERT INTO CreditCardEntryType (account_subtype_id)
SELECT account_subtype_id
FROM AccountSubtype
WHERE account_subtype IN (
        'DEBT',
...
INSERT INTO StockAccountEntryType (account_subtype_id)
SELECT account_subtype_id
FROM AccountSubtype
WHERE account_subtype IN (
        'STOCK',
...
INSERT INTO GicAccountSubtype (account_subtype_id)
SELECT account_subtype_id
FROM AccountSubtype
WHERE account_subtype IN ('PRINCIPAL', 'INTEREST', 'OPEN-BALANCE');
...
INSERT INTO IncomeAccountSubtype (account_subtype_id)
SELECT account_subtype_id
FROM AccountSubtype
WHERE account_subtype IN (
        'CPP',
...
SELECT account_subtype_id
FROM CashAccountEntryType
UNION
SELECT account_subtype_id
FROM CreditCardEntryType
UNION
SELECT account_subtype_id
FROM IncomeAccountSubtype
UNION
SELECT account_subtype_id
FROM GicAccountSubtype
UNION
SELECT account_subtype_id
FROM StockAccountEntryType;
...
CREATE TABLE DistributionTaxBreakdownType (
    distribution_tax_breakdown_type_id INTEGER NOT NULL PRIMARY KEY,
    distribution_tax_breakdown_type TEXT NOT NULL UNIQUE
) STRICT;

INSERT INTO DistributionTaxBreakdownType(distribution_tax_breakdown_type)
VALUES -- TODO? wait until I buy foreign stocks (very unlikely)
    ('FOREIGN'),
    -- the actual amount per share
    -- i.e. total distribution (cash/non-cash) = gain - tax
    ('RATE'),
    -- is a percentage relative to the distribution amount
    -- i.e. gain - tax = 100 (as percent)
    ('PERCENT');

-- tax bookkeeping for ETF distribution (for T3 form)
CREATE TABLE Distribution (
    security_id INTEGER NOT NULL,
    -- an indicator used for ordering distributions; don't have to be contiguous
    distribution_id INTEGER NOT NULL REFERENCES SECURITY(security_id),
    ...
    -- T3 tax breakdown
    distribution_tax_breakdown_type_id INTEGER NOT NULL REFERENCES DistributionTaxBreakdownType(distribution_tax_breakdown_type_id),
    -- box 21
    capital_gain TEXT NOT NULL,
    -- box 52
    capital_gain_pre_20240624 TEXT NOT NULL,
    -- box 53, for tax change of capital gain raising from 50% to 66%
    capital_gain_post_20240624 TEXT NOT NULL,
    -- box 49
    ...
    -- distribution from non-CAD securities, not part of T3; for reference and sanity check (to balance out cash distribution in the check)
    foreign_distribution TEXT NOT NULL,
    PRIMARY KEY (security_id, distribution_id)
) STRICT;
