CREATE VIEW CreditCardView AS WITH LastPayment AS (
    ...
    GROUP BY credit_card_holder_id
)
SELECT first_name,
    last_name,
...
FROM (
        SELECT DISTINCT cash_account_holder_id
        FROM CreditCardPadSource
    )
...
WHERE account_type_id IN (
    ...
    AND account_kind IN ('ASSET', 'LIABILITIES')
    AND (
        transaction_id IN (
            SELECT transaction_id
            FROM FinancialEntry
                ...
                INNER JOIN AccountSubtype USING (account_subtype_id)
                INNER JOIN AccountKind USING (account_kind_id)
            WHERE account_subtype_id NOT IN (
                    SELECT account_subtype_id
                    FROM AllAccountSubtypes
                    UNION
                    SELECT account_subtype_id
                    FROM NonCashbackSubtypes
                )
            EXCEPT
            SELECT -- exclude transactions related to prepaid credits (there's a separate query for PrepaidAccount)
                transaction_id
            FROM FinancialEntry
                INNER JOIN Account USING (account_id)
...
FROM FinancialEntry
    ...
    INNER JOIN AccountSubtype USING (account_subtype_id)
    INNER JOIN AccountKind USING (account_kind_id)
WHERE -- we're interested in cashback for *expense* (i.e. crediting asset accounts)
    credit IS NOT NULL
    ...
    transaction_id IN (
        ...
        FROM FinancialEntry
            ...
            INNER JOIN PrepaidAccount USING (account_type_id)
        EXCEPT
        SELECT -- include transactions that related to prepaid credit spendings
            transaction_id
        FROM FinancialEntry
            INNER JOIN Account USING (account_id)
            INNER JOIN AccountSubtype USING (account_subtype_id)
...
-- quick test
SELECT *
FROM CashbackTransactionBalance;
...
WHERE account_type_id <> (
        SELECT account_type_id
        FROM CreditCardProduct
            INNER JOIN Institution USING (institution_id)
        WHERE institution_name = 'American Express'
    )
GROUP BY transaction_id,
    store_id;
...
SELECT transaction_id,
    ...
    CashbackWithAmex.cashback_rate AS incl_amex_cashback_rate,
    ROUND(balance * CashbackWithAmex.cashback_rate, 2) AS incl_amex_cashback,
    CashbackNoAmex.cashback_rate AS excl_amex_cashback_rate,
    ROUND(balance * CashbackNoAmex.cashback_rate, 2) AS excl_amex_cashback
FROM CashbackTransactionBalance Actual
    LEFT JOIN CashbackNoAmex USING (transaction_id)
    LEFT JOIN CashbackWithAmex USING (transaction_id);

-- quick test
SELECT *
FROM CashbackEstimate;
...
SELECT CAST(
    ...
    SUM(
        (
            incl_amex_cashback_rate - excl_amex_cashback_rate
        ) * balance
    ) / SUM (balance) AS extra_cashback_rate,
    -- Compute the theoretical cashback if I used all cards including Amex.
    SUM(incl_amex_cashback) AS incl_amex_cashback,
    -- Compute the theoretical cashback if I used all cards excluding Amex.
    SUM(excl_amex_cashback) AS excl_amex_cashback,
    -- Compute the theoretical extra cashback earned by using Amex compared to not using it.
    SUM(incl_amex_cashback - excl_amex_cashback) AS extra_cashback,
    -- Compute the theoretical extra cashback earned by using Amex compared to not using it, net of fees.
    SUM(incl_amex_cashback - excl_amex_cashback) - (
        SELECT annual_fee
        FROM CreditCardProduct
            INNER JOIN Institution USING (institution_id)
        WHERE institution_name = 'American Express'
    ...
    SUM(actual_cashback - incl_amex_cashback) AS missed_opportunities
FROM CashbackEstimate
GROUP BY strftime(
        '%Y-%m',
