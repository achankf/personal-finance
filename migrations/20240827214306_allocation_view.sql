...
-- quick test
SELECT *
FROM NormalizedFixedIncomeBalance;
...
FROM NormalizedFixedIncomeBalance
    INNER JOIN AssetClass USING (person_id)
    INNER JOIN AssetClassName USING (asset_class_name_id)
WHERE asset_class_name = 'Fixed Income';

-- quick test
SELECT *
FROM BalanceByAssetClass;
...
-- quick test
SELECT *
FROM TotalAssetBalance;

CREATE VIEW AllocationView AS WITH RebalanceRate AS (
    SELECT COALESCE(liquid_balance, 0.0) / liquid_sum AS liquid_rate,
...
FROM RebalanceRate
WHERE current_rebalance_amount <> 0
