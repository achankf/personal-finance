...
use db::Db;
use transaction::{
    Account, AccountSubtype, AssetAllocation, AssetClassName, CashbackCard, CashbackCategory,
    CashbackCategoryName, Currency, Distribution, Exchange, Institution, Person, PrepaidAccount,
    Security, Store, StoreCashbackMapping, TaxShelterType, MyTransaction, TransactionStore,
};
        ...
        match self {
            Self::All {
                ...
                transaction
                    ...
                    .await?;
                transaction
                    .upsert_all::<Distribution>(&csv_path.join("distribution.csv"))
                    .await?;
                transaction
                ...
                transaction
                    ...
                    .await?;
                transaction
                    .upsert_all::<CashbackCard>(&csv_path.join("cashback_card.csv"))
                    .await?;
                transaction
                ...
                transaction
                    .sync_pending_transactions(&csv_path.join("pending_transaction.csv"))
                    .await?;
            ...
            Self::CashbackCard { csv_path } => {
                transaction.upsert_all::<CashbackCard>(&csv_path).await?;
            }
            Self::Distribution { csv_path } => {
                transaction.upsert_all::<Distribution>(&csv_path).await?;
            }
        };
