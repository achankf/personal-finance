...
use transaction::{AssetRebalance, CreditCardPadInjection, EmergencyRebalance, MyTransaction};

async fn print_account_last_update(transaction: &mut MyTransaction<'_>) -> SqlResult<()> {
    #[derive(Tabled)]
...
async fn print_emergency_injecion(transaction: &mut MyTransaction<'_>) -> SqlResult<()> {
    #[derive(Tabled)]
...
pub async fn print_current_credit(transaction: &mut MyTransaction<'_>) -> SqlResult<()> {
...
async fn print_rebalance(transaction: &mut MyTransaction<'_>) -> SqlResult<()> {
    #[derive(Tabled)]
    ...
    let results = results.into_iter().map(RebalanceFormatted::from);
