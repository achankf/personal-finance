...
use transaction::{Acb, Transaction};
...
pub async fn report_acb(
    ...
    if year > this_year {
        return Err(
            "No crytstal ball error (though I'm getting there): unable to show acb for the future"
                .into(),
        );
    }
    ...
    println!("{}", format!("Breakdown By Security").bold());
    println!(
        "{}",
        Table::new(formatted)
            .with(Style::rounded())
