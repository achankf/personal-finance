...
use transaction::{
    ...
    DistributionRefSumZeroCheckResult, MyTransaction,
};

pub async fn print_distribution_check(
    ...
    if !bad_distribution_refs.is_empty() {
        ...
        impl From<DistributionRefSumZeroCheckResult> for RecordFormatted {
            fn from(value: DistributionRefSumZeroCheckResult) -> Self {
                Self {
                    ...
                    total_distribution: value.total_distribution.to_string(),
        ...
        println!(
            "{}",
            format!("@future me: tax breakdown will be available by next year's March, so the table could show warning for this year's entries.")
                .to_string()
                .yellow()
                .bold()
        );
        println!(
            "{}",
            format!("CDS Tax Breakdown Service: https://services.cds.ca/applications/taxforms/taxforms.nsf/Pages/-EN-LimitedPartnershipsandIncomeTrusts?Open")
                .to_string()
                .yellow()
                .bold()
        );
    ...
    if !bad_distributions.financial_entry_errors.is_empty() {
        ...
        println!(
            ...
            Table::new(formatted)
                .with(Style::rounded())
                .with(Columns::new(5..=6).modify().with(Alignment::right()))
                .to_string()
        );
        println!();
    }
